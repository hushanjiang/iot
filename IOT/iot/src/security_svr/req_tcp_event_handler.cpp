...
int Req_TCP_Event_Handler::handle_input(int fd)
{
	...
	while(pos != std::string::npos)
	{
		...
		else if(req_src.size() > MAX_MSG_LEN)
		{
			...
			Msg_Oper::send_msg(fd, session_id, "", 0, msg_tag, ERR_REACH_MAX_MSG, "the req reach max len.");
		}
		else
		{
			//处理正常请求
			...
			{	
				...
				if(nRet != 0)
				{
					XCP_LOGGER_ERROR(&g_logger, "push req into req mgt failed, ret:%d, req(%u):%s\n", 
						nRet, buf_len, buf);
					Msg_Oper::send_msg(fd, session_id, "", 0, msg_tag, ERR_PUSH_QUEUE_FAIL, "push req into req mgt failed.");
				}
			...
			{		
				...
				Msg_Oper::send_msg(fd, session_id, "", 0, msg_tag, ERR_QUEUE_FULL, "req mgt is full.");
			}
