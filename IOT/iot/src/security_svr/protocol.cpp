...
int XProtocol::create_security_channel_req(const std::string &req, std::string &id, std::string &key, std::string &err_info)
{
	err_info = "";
	...
	if(!json_object_object_get_ex(params, "id", &_id))
	{
	...
	if(!json_object_is_type(_id, json_type_string))
	{
		XCP_LOGGER_INFO(&g_logger, "it is invalid req, id isn't string, req(%u):%s\n", req.size(), req.c_str());
		err_info = "it is invalid req, id isn't string.";
		json_object_put(root);
		return -1;

	}
	id = json_object_get_string(_id);

	//iot_key
	json_object *_iot_key = NULL;
	if(!json_object_object_get_ex(params, "iot_key", &_iot_key))
	{
		XCP_LOGGER_INFO(&g_logger, "it is invalid req, no iot_key, req(%u):%s\n", req.size(), req.c_str());
		err_info = "it is invalid req, no iot_key.";
		json_object_put(root);
		return -1;
	}
	if(!json_object_is_type(_iot_key, json_type_string))
	{
	...
	key = json_object_get_string(_iot_key);
...
int XProtocol::refresh_security_channel_req(const std::string &req, std::string &id, std::string &err_info)
{
	err_info = "";
