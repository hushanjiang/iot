...
int Conf_Mgt::refresh()
{
	...
	//id
	XML_Node node;
	nRet = _parser.get_node("security_svr/system/id", node);
	if(nRet != 0)
	{
	...
	//ip
	nRet = _parser.get_node("security_svr/system/ip", node);
	if(nRet != 0)
	{
	...
	//port
	nRet = _parser.get_node("security_svr/system/port", node);
	if(nRet != 0)
	{
	...
	//thr_num
	nRet = _parser.get_node("security_svr/system/thr_num", node);
	if(nRet != 0)
	{
	...
	//max_queue_size
	nRet = _parser.get_node("security_svr/system/max_queue_size", node);
	if(nRet != 0)
	{
	...
	//TZ
	nRet = _parser.get_node("security_svr/system/TZ", node);
	if(nRet != 0)
	{
	...
	nRet = _parser.get_nodes("security_svr/mysql/svr", vecNode);
	if(nRet != 0)
	{
	...
	vecNode.clear();
	nRet = _parser.get_nodes("security_svr/redis/svr", vecNode);
	if(nRet != 0)
	{
