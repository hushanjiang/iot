...
int XProtocol::get_page_params(const std::string &req, unsigned int &size, unsigned int &beginAt)
{
	enum json_tokener_error jerr;
	json_object *root = json_tokener_parse_verbose(req.c_str(), &jerr);
	if(jerr != json_tokener_success)
	...
	if(!json_object_object_get_ex(params, "page", &page))
	{
		XCP_LOGGER_INFO(&g_logger, "cannot find page in params, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}
	if(!json_object_is_type(page, json_type_object))
	{
		XCP_LOGGER_INFO(&g_logger, "it is invalid req, page isn't json type, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}

	json_object *_size = NULL;
	if(!json_object_object_get_ex(page, "size", &_size))
	{
		XCP_LOGGER_INFO(&g_logger, "cannot find size in page, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}
	if(!json_object_is_type(_size, json_type_int))
	{
		XCP_LOGGER_INFO(&g_logger, "it is invalid req, size isn't integer type, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}
	size = (unsigned int)json_object_get_int(_size);

	json_object *_begin = NULL;
	if(!json_object_object_get_ex(page, "begin", &_begin))
	{
		XCP_LOGGER_INFO(&g_logger, "cannot find begin in page, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}
	if(!json_object_is_type(_begin, json_type_int))
	{
		XCP_LOGGER_INFO(&g_logger, "it is invalid req, begin isn't integer type, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}
	beginAt= (unsigned int)json_object_get_int(_begin);
...
int XProtocol::get_room_orders_params(const std::string &req, std::vector<StRoomInfo> &infos)
{
	enum json_tokener_error jerr;
	json_object *root = json_tokener_parse_verbose(req.c_str(), &jerr);
	if(jerr != json_tokener_success)
	...
	json_object *orders = NULL;
	if(!json_object_object_get_ex(params, "list", &orders))
	{
		XCP_LOGGER_INFO(&g_logger, "cannot find list in params, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}
	if(!json_object_is_type(orders, json_type_array))
	{
		XCP_LOGGER_INFO(&g_logger, "it is invalid req, list isn't array type, req(%u):%s\n", req.size(), req.c_str());
		json_object_put(root);
		return -1;
	}

	int size = json_object_array_length(orders);
	for (int i = 0; i<size; i++)
	{
		json_object *_info = json_object_array_get_idx(orders, i);
		StRoomInfo info;
		
		json_object *_roomId;
		if (!json_object_object_get_ex(_info , "room_id", &_roomId))
		{
			XCP_LOGGER_INFO(&g_logger, "it is invalid req, no room_id in list of the %dth element\n", i);
			json_object_put(root);
			return -1;
		}
		if(!json_object_is_type(_roomId, json_type_int))
		{
			XCP_LOGGER_INFO(&g_logger, "it is invalid req, room_id isn't unsigned long type, req(%u):%s\n", req.size(), req.c_str());
			json_object_put(root);
			return -1;
		}
		info.id = (uint64)json_object_get_int64(_roomId);
		
		json_object *_order;
		if (!json_object_object_get_ex(_info , "order", &_order))
		{
			XCP_LOGGER_INFO(&g_logger, "it is invalid req, no order in list of the %dth element\n", i);
			json_object_put(root);
			return -1;
		}
		if(!json_object_is_type(_order, json_type_int))
		{
			XCP_LOGGER_INFO(&g_logger, "it is invalid req, order isn't integer tyep, req(%u):%s\n", req.size(), req.c_str());
			json_object_put(root);
			return -1;
		}
		info.orderNum = (unsigned int)json_object_get_int(_order);
...
std::string XProtocol::add_room_result(const unsigned long long &roomId)
{
	char buf[512] = {0};
	memset(buf, 0x0, 512); 
	snprintf(buf, 512, "{\"room_id\":%llu}", roomId);
...
std::string XProtocol::get_room_info_result(const StRoomInfo &info)
{
	char buf[512] = {0};
	memset(buf, 0x0, 512); 
	snprintf(buf, 512, "{\"room_id\":%llu, \"family_id\":%llu, \"room_name\": \"%s\", \"order\":%llu, \"created_at\":%llu, \"updated_at\":%llu}",
...
std::string XProtocol::get_device_info_result(const StDeviceInfo &info)
{
	char buf[512] = {0};
	memset(buf, 0x0, 512); 
	snprintf(buf, 512, "{\"device_id\":%llu, \"device_uuid\":\"%s\", \"family_id\": \"%llu\", \"room_id\": \"%llu\", \"device_category_id\":%llu, \"device_name\":\"%s\", \"device_attr_ext\":\"%s\", \"created_at\":%llu, \"updated_at\":%llu}",
...
std::string XProtocol::get_device_status_result(const StDeviceStatus &status)
{
	char buf[512] = {0};
	memset(buf, 0x0, 512); 
	snprintf(buf, 512, "{\"report_id\":%llu, \"report_type_id\":%u, \"device_id\": \"%llu\", \"family_id\": \"%llu\", \"report_msg\":\"%s\", \"report_created_at\":%llu}",
		status.id, status.type, status.deviceId, status.familyId, status.msg.c_str(), status.createAt);
...
std::string XProtocol::get_status_list_result(const std::vector<StDeviceStatus> &status, const unsigned int &left)
{
	std::ostringstream result;
	result << "{\"list\":[";

	bool isFirstElement = true;
	std::vector<StDeviceStatus>::const_iterator end = status.end();
	for(std::vector<StDeviceStatus>::const_iterator it = status.begin(); it != end; it++)
	{
		if(!isFirstElement)
		{
			result << ",";
