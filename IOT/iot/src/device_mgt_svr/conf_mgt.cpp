...
int Conf_Mgt::refresh()
{
	...
	//id
	XML_Node node;
	nRet = _parser.get_node("device_mgt_svr/system/id", node);
	if(nRet != 0)
	{
	...
	//ip
	nRet = _parser.get_node("device_mgt_svr/system/ip", node);
	if(nRet != 0)
	{
	...
	//port
	nRet = _parser.get_node("device_mgt_svr/system/port", node);
	if(nRet != 0)
	{
	...
	//Message ID�������� [svr id]_[ip]_[port]
	sysInfo._new_id = format("%s_%s_%u", sysInfo._id.c_str(), sysInfo._ip.c_str(), sysInfo._port);
	
	//thr_num
	nRet = _parser.get_node("device_mgt_svr/system/thr_num", node);
	if(nRet != 0)
	{
	...
	//max_queue_size
	nRet = _parser.get_node("device_mgt_svr/system/max_queue_size", node);
	if(nRet != 0)
	{
	...
	//TZ
	nRet = _parser.get_node("device_mgt_svr/system/TZ", node);
	if(nRet != 0)
	{
	...
	nRet = _parser.get_nodes("device_mgt_svr/conf/svr", vecNode);
	if(nRet != 0)
	{
	...
	vecNode.clear();
	nRet = _parser.get_nodes("device_mgt_svr/mysql/svr", vecNode);
	if(nRet != 0)
	{
	...
	vecNode.clear();
	nRet = _parser.get_nodes("device_mgt_svr/redis/svr", vecNode);
	if(nRet != 0)
	{
