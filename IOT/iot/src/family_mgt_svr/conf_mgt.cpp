...
int Conf_Mgt::init_conn_mgt()
{
	...
	//push svr
	RSP_TCP_Event_Handler *handler_push = new RSP_TCP_Event_Handler(ST_PUSH);
	nRet = g_push_conn.init(&svrs, handler_push, true);
	if(nRet != 0)
	{
		printf("init push svr conn.\n");
		return nRet;
	}
...
int Conf_Mgt::refresh()
{
	...
	//id
	XML_Node node;
	nRet = _parser.get_node("family_mgt_svr/system/id", node);
	if(nRet != 0)
	{
	...
	//ip
	nRet = _parser.get_node("family_mgt_svr/system/ip", node);
	if(nRet != 0)
	{
	...
	//port
	nRet = _parser.get_node("family_mgt_svr/system/port", node);
	if(nRet != 0)
	{
	...
	//thr_num
	nRet = _parser.get_node("family_mgt_svr/system/thr_num", node);
	if(nRet != 0)
	{
	...
	//max_queue_size
	nRet = _parser.get_node("family_mgt_svr/system/max_queue_size", node);
	if(nRet != 0)
	{
	...
	//TZ
	nRet = _parser.get_node("family_mgt_svr/system/TZ", node);
	if(nRet != 0)
	{
	...
	nRet = _parser.get_nodes("family_mgt_svr/conf/svr", vecNode);
	if(nRet != 0)
	{
	...
	vecNode.clear();
	nRet = _parser.get_nodes("family_mgt_svr/mysql/svr", vecNode);
	if(nRet != 0)
	{
	...
	vecNode.clear();
	nRet = _parser.get_nodes("family_mgt_svr/redis/svr", vecNode);
	if(nRet != 0)
	{
