...
int Req_TCP_Event_Handler::handle_accept(int fd)
{
	...
	XCP_LOGGER_INFO(&g_logger, "accept from svr(fd:%d), %s:%u --> %s:%u\n", 
...
int Req_TCP_Event_Handler::handle_input(int fd)
{
	...
	if(nRet == 0)
	{
		XCP_LOGGER_INFO(&g_logger, "rcv close form svr(fd:%d, peer:%s:%u), ret:%d\n", fd, ip.c_str(), port, nRet);
		return -1;
	}
	else if(nRet == 1)
	{
		//XCP_LOGGER_INFO(&g_logger, "rcv success from svr(fd:%d), req(%u):%s\n", fd, buf_len, buf); 
	}
	else
	{
		XCP_LOGGER_INFO(&g_logger, "rcv failed from svr(fd:%d, peer:%s:%u), ret:%d\n", fd, ip.c_str(), port, nRet);
		return 0;
	}
	buf[buf_len] = '\0';
...
int Req_TCP_Event_Handler::handle_close(int fd)
{
	...
	XCP_LOGGER_INFO(&g_logger, "close from svr(fd:%d), %s:%u --> %s:%u\n", 
		fd, remote_ip.c_str(), remote_port, local_ip.c_str(), local_port);
