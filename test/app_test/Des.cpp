...
static unsigned char * byte2bit(unsigned char byte[64], unsigned char bit[8])
...
/*  				   bit2byte 					  */
/*----------------------------------------------------*/
static unsigned char * bit2byte(unsigned char bit[8], unsigned char byte[64])
...
/*  				change the key  				*/
/*--------------------------------------------------*/
static void keychange(unsigned char oldkey[8], unsigned char newkey[16][8])
{
	int i = 0, j = 0, k = 0 ;
	int pc_1[56] =
	{
...
/*  			   S_replace						*/
/*--------------------------------------------------*/
static void s_replace(unsigned char s_bit[8])
{
	int p[32] =
	{
...
/*  				data encryption 				*/
/*--------------------------------------------------*/
static void endes(unsigned char m_bit[8], unsigned char k_bit[8], unsigned char e_bit[8])
{
	int ip[64] =
	{
...
/*  				data uncryption 				*/
/*--------------------------------------------------*/
static void undes(unsigned char m_bit[8], unsigned char k_bit[8], unsigned char e_bit[8])
{
	int ip[64] =
	{
		58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54,
...
void commonutil::DesEnc(unsigned char *outdata, unsigned char *indata, int datalen, const unsigned char *key)
{
	int i, blklen;
	unsigned char Key[8];
	...
	blklen = datalen / 8;
	//	for(i = 0; i < blklen; i++, indata += 8, outdata += 8)
	//		endes(indata , Key , outdata);
	for(i = 0; i < blklen; i++)
	{
...
void commonutil::DesDec(unsigned char *outdata, unsigned char *indata, int datalen, const unsigned char *key)
{
	int i, blklen;
	unsigned char Key[8];
	unsigned char temp[1024]={0};
